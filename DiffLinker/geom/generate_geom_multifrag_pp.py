...
def run_pp(geom_json_path, output_path, min_size, mmpa, brics):
    if ".json" == os.path.splitext(geom_json_path)[1]:
        with open(geom_json_path) as f:
            geom_json = json.load(f)
        all_smiles = list(geom_json.keys())
    ...
    inputs_df = pd.DataFrame([all_smiles]).T
    inputs_df.columns = ["smiles"]
    inputs_df["mmpa"] = mmpa
    inputs_df["brics"] = brics
    inputs_df["min_size"] = min_size
    inputs = inputs_df.to_dict('records')
    with multiprocessing.Pool(os.cpu_count() - 2) as pool:
        many_tables = pool.map_async(wrap, inputs).get()
    table = pd.concat(many_tables, axis=0)
    table = table.drop_duplicates(['molecule', 'linker'])
    table.to_csv(output_path, index=False)
