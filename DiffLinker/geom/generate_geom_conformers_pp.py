...
def wrap(inputs):
    geom_json_path = inputs["geom_json_path"]
    geom_root_path = inputs["geom_root_path"]
    generated_splits_path = inputs["generated_splits_path"]
    pickle_path = inputs["pickle_path"]
    smi = inputs["smiles"]
...
def generate_conformers(geom_json_path, geom_root_path, generated_splits_path, output_path):
    ...
    inputs_df = pd.DataFrame([smiles]).T
    inputs_df.columns = ["smiles"]
    inputs_df["geom_json_path"] = geom_json_path
    inputs_df["geom_root_path"] = geom_root_path
    inputs_df["generated_splits_path"] = generated_splits_path
    inputs_df["pickle_path"] = [geom_json[smi].get('pickle_path') for smi in inputs_df["smiles"]]
    inputs = inputs_df.to_dict('records')
    with multiprocessing.Pool(os.cpu_count() - 2) as pool:
        molecules = pool.map_async(wrap, inputs).get()
    molecules = list(filter(None, molecules))
